...
export function withDerived(draws: Draw[]): Draw[] {
  return (draws || []).map((dr) => {
    ...
    const bySym: Record<string, LabItem> = {};
    for (const it of items) if (it.symbol) bySym[it.symbol] = it;
    const byAnalysis = (a: string) => items.find((it) => it.analysis === a);

    const tb = bySym["T-BIL"];
    const db = bySym["D-BIL"];
    if (tb && db && !bySym["I-BIL"] && tb.us.value != null && db.us.value != null && tb.us.unit === db.us.unit) {
    ...
    const alb = bySym["ALB"] ?? byAnalysis("Albumin");
    if (tp && alb && !bySym["GLOB"] && tp.us.value != null && alb.us.value != null && tp.us.unit === alb.us.unit) {
